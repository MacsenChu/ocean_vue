...
这个动画向下展示的动画，又是我一个踩坑的地方，我是用的Vue的transition组件，照着官网写各种CSS，最后需要的是一个height: 0px到height：这个元素的高。可是这三个头像弹出来的动画都不一边高，那么只能全部用钩子函数来写动画，用clientHeight获取元素的高……就这样挣扎了许久，动画写的多多少少还是那么生硬。最后我看到了element有自带的动画……………………
